...
void TcpClient::connectToServer() {
    socket->connectToHost("10.10.20.109", 10010);  //socket->connectToHost("10.10.20.109", 20006);
...
void TcpClient::sendJson(const QJsonObject &json) {
    ...
    if (socket && socket->state() == QAbstractSocket::ConnectedState) {
        ...
        // waitForBytesWritten()으로 전송이 완료될 때까지 대기
        if (!socket->waitForBytesWritten(3000)) {
            qWarning() << "데이터 전송 실패: " << socket->errorString();
        }
        ...
        qWarning() << "소켓이 nullptr이거나 연결되어 있지 않습니다.";
    }
}
...
void TcpClient::onReadyRead() {
    ...
    // qDebug() << "현재 버퍼:" << QString::fromUtf8(buffer);
    ...
    if (error.error == QJsonParseError::NoError) {
        ...
        qDebug() << "문제 있는 버퍼:" << QString::fromUtf8(buffer);
