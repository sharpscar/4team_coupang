        ...
        else if (signal == "login") // 로그인(USER & STORE)
        {
            ...
            if (role == "user") {
                ...
                if (mysql_query(conn, login_query.c_str()) == 0) {
                    ...
                    send_json(client_sock, {{"signal", "login_result"}, {"result", "login fail"}});
                }
            }
            ...
            if(role=="user") {
                ...
                if (mysql_query(conn, query.c_str()) == 0)
                {
                    ...
                    if (result != nullptr)
                    {
                        ...
                        cout << num_rows;
                        if (num_rows > 0) {
                            send_json(client_sock, {{"signal", "find_pw_result"},{"result", "success"}, {"user_id",pw}});
                        } else {
                            send_json(client_sock, {{"signal", "find_pw_result"},{"result", "fail"}});
                        }
                        mysql_free_result(result);  // 메모리 해제
                    }
                    else {
            ...
            auto escape_quotes = [](const std::string& input) {
                ...
                if (mysql_query(conn, query.c_str()) == 0)
                {
                    ...
                    if (result != nullptr)
                    {
                        ...
                        cout << num_rows;
                        if (num_rows > 0) {
                            send_json(client_sock, {{"signal", "find_info_result"},{"result", "success"}, {"user_id",email}, {"name", name}, {"pnum", pnum}, {"birth", birth}});
                        } else {
                            send_json(client_sock, {{"signal", "find_info_result"},{"result", "fail"}});
                        }
                        mysql_free_result(result);  // 메모리 해제
                    }
                    else {
