    ...
    try {
        ...
        if (signal == "create") {
            ...
            if (mysql_query(conn, query.c_str()) == 0) {
                ...
                send_json(client_sock, { {"signal", "main_data"}, {"users", store} });
            } else {
            ...
            else if (role == "store")
            {
                ...
                if (mysql_query(conn, login_query.c_str()) == 0)
                {
                    ...
                    if (mysql_query(conn, query2.c_str()) == 0) {
                        ...
                        mysql_free_result(result);
                        send_json(client_sock, { {"signal", "response_store_login"}, {"store_login_result", "success"}, {"store_info", store} });
                    } else {
        ...
        else if (signal == "save_cart") // 메뉴 장바구니에 담기
        {
            ...
            string menus = j["menus"];

            cout << "user_id" << user_id <<endl;
            cout << "store_name" << store_name <<endl;
